...
class LogsDataProcessor:
    ...
    def _load_df(self):
        ...
        df = df[[self.case_id_col, self.activity_col, self.label_col, self.lpms_frequency, self.lpms_binary, self.time_col]]
        df.columns = [self.case_id_col, self.activity_col, self.label_col, self.lpms_frequency, self.lpms_binary, self.time_col]
    ...
    def _outcome_helper_func(self, df, Max_prefix_length):
        ...
        processed_df = pd.DataFrame(columns=[self.base_case_id, self.case_id_col, "prefix", "k", self.label_col, ])
        ...
        for _, case in enumerate(unique_cases):
            ...
            prefix = " ".join(act)
            processed_df.at[idx, self.base_case_id] = case.split("_")[0]
            processed_df.at[idx, self.case_id_col] = case
            processed_df.at[idx, "prefix"] = prefix
            if self._LPMs:
                for ll in lpm_measures:
                    LPM_seq = " ".join(str(lpms[ll]))
    ...
    def _process_outcome(self, df, train_list, test_list, Max_prefix_length):
        ...
        train_df = processed_df[processed_df[self.base_case_id].isin(train_list)]
        test_df = processed_df[processed_df[self.base_case_id].isin(test_list)]
    ...
    def test_train_spliting(self, df, train_ratio, trunc_ratio):
        if trunc_ratio > 1:
        ...
        Prefixes = []
        Labels = []
        for case in df[self.base_case_id].unique():
            Cases.append(case)
            Prefixes.append(df.groupby(self.base_case_id)['case_length'].max())
            Labels.append(list(df[df[self.base_case_id] == case][self.label_col].unique())[0])

        Case_indexes = pd.DataFrame({self.base_case_id: Cases, "prefix length": Prefixes, self.label_col: Labels})
        ...
        train_list = train[self.base_case_id]
        test_list = test[self.base_case_id]
